/**
  ...
  note that this post cannot return full rapid functionality, if you need that, buy a license!
 A few of Davids mods changed
*/
...
longDescription = "Generic milling post for WorkBee CNC Machine.";
...
var feedOutput = createVariable({prefix:"F", force:true}, feedFormat); //swarfer if duet requires feed on every line then set to true
...
var linmove = 1;        // swarfer linear move mode - required to be initialized
...
function onSection() {
  ...
  writeln("");
  if(isFirstSection()) { // For Stuart Wilson 24:05:20
  writeln("G53 G0 Z90");
  }
  writeln("");
...
// wcs
// Modified to suit updated ooznest WCS convention with G53 being machine coordinates.
      ...
      if (x || y || z) {
         if (!haveRapid && z)  // if z is changing
            if (_z < retractHeight) // compare it to retractHeight, below that is G1, >= is G0
               ...
               linmove = 0;
         if (linmove == 0)
            f = ""; // no feed on G0 lines            
         writeBlock(gMotionModal.format(linmove), x, y, z, f);
         if (linmove == 0)
            feedOutput.reset(); // force feedrate on next G1 line
         }
      }
  } else if (f) {
...
function onClose() {
   //home Z at end of job, just a nice little touch so you know it finished instead of failed
 {
  writeln("G53 G0 Z90");
  }
  writeln("");
   //writeBlock(mFormat.format(30)); // swarfer technically all gcode programs should have an end program code
}

//swarfer need this to account for people using non top-of-work origins
function onParameter(name, value) {
   if ( (name.indexOf("retractHeight") >= 0)  && (name.indexOf("value") >= 0 ) ) { 
      retractHeight = value;
   }
}
